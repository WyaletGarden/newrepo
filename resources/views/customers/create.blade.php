                    ...
                    <form action="{{ route('customers.store') }}" method="POST">
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="address" id="address" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            ...
                            <div>
                                ...
                                <select name="customer_type" id="customer_type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('customer_type') border-red-500 @enderror">
                                    <option value="regular" {{ old('customer_type') === 'regular' ? 'selected' : '' }}>Khách hàng thường</option>
                                    <option value="vip" {{ old('customer_type') === 'vip' ? 'selected' : '' }}>Khách hàng VIP</option>
                                    <option value="wholesale" {{ old('customer_type') === 'wholesale' ? 'selected' : '' }}>Khách hàng sỉ</option>
                                </select>
                                @error('customer_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
